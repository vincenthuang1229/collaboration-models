;;; Model5 :Homophily Model
;;; The process of collaboration is infected by five parameters: p, q, experience bound, global xp distribution and homophily;
...
;;; two players will not collaborate with each other if their experience gap is beyond experience bound,for example 10.

globals
  [
    global_m_nmxp  ;; mean of experience of all men at each time t
    global_w_nmxp  ;; mean of experience of all women at each time t
  ]
...
turtles-own
  [
    ...
    xp           ;; the value of players' experince
...
to xp-normalization
  ;; create sequence of players' experience follows normal distribution
...
to calculate_global_xp
  ;; calculate mean of mens' and womens' experience and normalization
  set global_m_nmxp mean [xp] of turtles with [male?] / ( mean [xp] of turtles with [female?] + mean [xp] of turtles with [male?])
  set global_w_nmxp mean [xp] of turtles with [female?] / ( mean [xp] of turtles with [female?] + mean [xp] of turtles with [male?])
end
...
to output
  ...
  if ticks = 5000  [ export-world "Trial5_5k.csv" ]
  if ticks = 10000 [ export-world "Trial5_10k.csv" ]
  if ticks = 15000 [ export-world "Trial5_15k.csv" ]
  if ticks = 20000 [ export-world "Trial5_20k.csv" ]
  if ticks = 25000 [ export-world "Trial5_25k.csv" ]
  if ticks = 30000 [ export-world "Trial5_30k.csv"
                     export-output "Output5.txt" ]
end
...
to collaborate
  ...
  ;; player1 tends to collaborate with player2 with probability of p1 based on player2's xp level and with probability g1 based on global xp distribution
  ;; player2 tends to collaborate with player1 with probability of p2 based on player1's xp level and with probability g1 based on global xp distribution
  ;; in addition, two players will not collaborate with each other if their experience gap is beyond experience bound: 10
  ;; there are 4 situation: M-M, M-F, F-M, F-F
  ;; situation 1: player1 and player2 are both men
  if [ male? ] of player1 and [ male? ] of player2
    [ ifelse player1 = player2
        [ collaborate ]
        [ ifelse random-float 1 > 0.7
            [ collaborate ]
            [ ifelse abs ([xp] of player1 - [xp] of player2) > 10
                [ collaborate ]
                [ ask player1
                    [ ifelse random-float 1 > global_m_nmxp
                        [ collaborate ]
  ...
  ;; situation 2: player1 is man and player2 is woman
  if [ male? ] of player1 and [ female? ] of player2
    [ ifelse player1 = player2
        [ collaborate ]
        [ ifelse random-float 1 > 0.3
            ...
            [ ifelse abs ([xp] of player1 - [xp] of player2) > 10
                [ collaborate ]
                [ ask player1
                    [ ifelse random-float 1 > global_w_nmxp
                        [ collaborate ]
                        [ ifelse random-float 1 > ([xp] of player2 / max [xp] of turtles)
                            [ collaborate ]
                            [ ask player2
  ...
  ;; situation 3: player1 is woman and player2 is man
  if [ female? ] of player1 and [ male? ] of player2
    [ ifelse player1 = player2
        [ collaborate ]
        [ ifelse random-float 1 > 0.3
            [ collaborate ]
  ...
  ;; situation 4: player1 and player2 are both women
  if [ female? ] of player1 and [ female? ] of player2
    [ ifelse player1 = player2
        [ collaborate ]
        [ ifelse random-float 1 > 0.7
            [ collaborate ]
