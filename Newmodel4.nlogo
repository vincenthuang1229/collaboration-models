;;; Model4 :Local Model
;;; The process of collaboration is infected by four parameters: p, q, experience bound and local experience distribution;
...
turtles-own
  [
    ...
    local_m_xp   ;; mean of experience level of a player's male neighbors
    local_w_xp   ;; mean of experience level of a player's female neighbors
...
to wire-node
  ...
  [ ;; make edges with the next two neighbors
    ;; this make a lattice with average degree of 8
    make-edge turtle n
...
to calculate_local_xp
   ask turtles
      [ if any? link-neighbors with [male?]
          [ set local_m_xp mean [xp] of link-neighbors with [male?] ]
        if any? link-neighbors with [female?]
          [ set local_w_xp mean [xp] of link-neighbors with [female?] ]
      ]
end

to xp-normalization
  ;; create sequence of players' experience follow normal distribution
...
to output
  ...
  if ticks = 5000  [ export-world "Trial4_5k.csv" ]
  if ticks = 10000 [ export-world "Trial4_10k.csv" ]
  if ticks = 15000 [ export-world "Trial4_15k.csv" ]
  if ticks = 20000 [ export-world "Trial4_20k.csv" ]
  if ticks = 25000 [ export-world "Trial4_25k.csv" ]
  if ticks = 30000 [ export-world "Trial4_30k.csv"
                     export-output "Output4.txt" ]
end
...
to collaborate
  ...
  ;; player1 tends to collaborate with player2 with probability of p1 based on player2's xp level and with probability g1 based on local xp distribution
  ;; player2 tends to collaborate with player1 with probability of p2 based on player1's xp level and with probability g1 based on local xp distribution
  ...
  if [ male? ] of player1 and [ male? ] of player2
    [ ifelse player1 = player2
        ...
        [ ifelse abs ([xp] of player1 - [xp] of player2) > 10
            [ collaborate ]
            [ ask player1
                [ ifelse random-float 1 > local_m_xp / ( local_m_xp + local_w_xp )
                    [ collaborate ]
  ...
  ;; situation 2: player1 is a man and player2 is a woman
  if [ male? ] of player1 and [ female? ] of player2
    [ ifelse player1 = player2
        ...
        [ ifelse abs ([xp] of player1 - [xp] of player2) > 10
            [ collaborate ]
            [ ask player1
                [ ifelse random-float 1 > local_w_xp / ( local_m_xp + local_w_xp )
                    [ collaborate ]
                    [ ifelse random-float 1 > ([xp] of player2 / max [xp] of turtles)
                        [ collaborate ]
                        [ ask player2
  ...
  ;; situation 3: player1 is a woman and player2 is a man
